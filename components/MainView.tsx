...
const MainView: React.FC<MainViewProps> = ({ 
  ...
  // Initializing log from chat messages whenever the active chat changes
  useEffect(() => {
    if (activeChat) {
  ...
  const handleLogChange = (newVal: string) => {
    ...
    const newMessages: Message[] = entries.map(entry => {
      ...
      if (colonIndex !== -1) {
        ...
        // Check if the author is the bot to assign assistant role
  ...
  const handleSend = async (overrideLog?: string) => {
    ...
    setIsTyping(true);
    try {
      // Create a snapshot for the AI to process based on current messages
      // Note: handleLogChange already updated activeChat.messages
  ...
  const handleRegen = async () => {
    ...
    if (logs.length > 0) {
      // Remove last entry if it's likely from the bot, or just remove last entry regardless for a 'regen' behavior
      logs.pop(); 
      const newLog = logs.join('\n\n');
      setEditableLog(newLog);
      handleLogChange(newLog);
      // Wait a tiny bit for state to prop and then send again (empty input)
      setTimeout(() => handleSend(newLog), 50);
    }
  };
  ...
  return (
    <div className="perchance-container pb-20">
      {/* Top Carousel with active highlighting */}
      <div className="flex gap-2 overflow-x-auto pb-4 mb-4 scrollbar-hide border-b border-[#222]">
        {characters.map(c => (
          <div 
            key={c.id} 
            className={`relative group shrink-0 w-44 border p-2 flex gap-2 cursor-pointer transition-colors ${activeChar.id === c.id ? 'bg-[#121215] border-zinc-600 shadow-[0_0_10px_rgba(39,39,42,0.5)]' : 'bg-[#0a0a0c] border-[#222] opacity-60 hover:opacity-100 hover:border-zinc-700'}`} 
          ...
          >
             ...
             <div className="min-w-0">
                ...
                <p className="text-[9px] text-zinc-500 line-clamp-2 leading-tight">{c.description || "No description..."}</p>
             </div>
      ...
      <div className="text-center mb-6">
         <button onClick={onCreateNew} className="pc-btn text-[10px] uppercase font-bold tracking-[0.2em] px-6 py-2 hover:bg-[#2a2a2e]">✨ generate a character</button>
      </div>
      ...
      <div className="relative">
        <textarea 
          className="pc-input min-h-[100px] font-serif leading-relaxed" 
      ...
      <div className="divider relative">
        — Chat Logs —
        <button onClick={handleUndo} className="absolute right-0 pc-btn text-[9px] flex items-center gap-1 bg-[#222] hover:bg-[#333] border-[#444] rounded px-2 py-0.5">↩ undo</button>
      </div>
      
      <div className="relative">
        <textarea 
          className="pc-input min-h-[450px] font-serif text-[15px] leading-relaxed p-6 bg-[#08080a] border-[#222] shadow-inner selection:bg-blue-500/20" 
          value={editableLog}
          onChange={e => setEditableLog(e.target.value)}
          onBlur={e => handleLogChange(e.target.value)}
          placeholder="The chat logs will show up here, and you can edit them. Add lines in 'Name: Content' format."
        />
        <div className="flex justify-center gap-4 mt-3">
           <button onClick={handleRegen} className="pc-btn text-[11px] flex items-center gap-2 px-6 py-1.5 border-[#333] hover:bg-zinc-800"><svg className="w-3 h-3" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2.5" d="M4 4v5h.582m15.356 2A8.001 8.001 0 004.582 9m0 0H9m11 11v-5h-.581m0 0a8.003 8.003 0 01-15.357-2m15.357 2H15" /></svg> regen</button>
           <button onClick={handleDeleteLast} className="pc-btn text-[11px] flex items-center gap-2 px-6 py-1.5 border-[#333] hover:bg-zinc-800"><svg className="w-3 h-3" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M19 7l-.867 12.142A2 2 0 0116.138 21H7.862a2 2 0 01-1.995-1.858L5 7m5 4v6m4-6v6m1-10V4a1 1 0 00-1-1h-4a1 1 0 00-1 1v3M4 7h16" /></svg> delete last</button>
        </div>
      </div>
      ...
      <div className="mt-10 space-y-4">
        <div className="relative">
          <textarea 
            className="pc-input min-h-[100px] p-5 bg-[#111] border-[#333] text-[14px] focus:ring-1 focus:ring-blue-500/20" 
            placeholder="Write here and tap send, or just tap send to let the AI continue..."
            value={inputText}
            onChange={e => setInputText(e.target.value)}
            onKeyDown={e => { if(e.key === 'Enter' && !e.shiftKey) { e.preventDefault(); handleSend(); } }}
          />
          <div className="flex items-center gap-1.5 absolute bottom-2 right-3 text-[10px] text-zinc-600 font-bold">
            <input type="checkbox" id="auto-improve" className="accent-blue-500" />
        ...
        <div className="flex justify-center">
           <button 
             onClick={() => handleSend()}
             disabled={isTyping}
             className="pc-btn pc-btn-primary px-20 py-5 text-[18px] uppercase tracking-[0.4em] flex items-center gap-4 shadow-[0_10px_30px_rgba(59,130,246,0.1)] active:translate-y-0.5 transition-transform"
           >
      ...
      <div className="flex flex-wrap justify-center gap-2 mt-8">
         {['Emma', 'Woman', 'Anon', 'Bot', 'Narrator'].map(name => (
           <button 
             ...
             className={`pc-btn text-[11px] px-4 py-2 border transition-all ${authorName === name ? 'bg-zinc-800 border-zinc-500 text-white shadow-md ring-1 ring-zinc-500/20' : 'border-[#222]'}`}
           >
             <span className={`w-1.5 h-1.5 rounded-full inline-block mr-2 ${authorName === name ? 'bg-blue-400 animate-pulse' : 'bg-zinc-700'}`}></span>
             {name}
           </button>
         ))}
         <button className="pc-btn text-[11px] px-3">+</button>
         <button onClick={() => { if(confirm("Clear entire chat log?")) handleLogChange(''); }} className="pc-btn text-[11px] px-3 hover:text-red-400"><svg className="w-3.5 h-3.5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2.5" d="M19 7l-.867 12.142A2 2 0 0116.138 21H7.862a2 2 0 01-1.995-1.858L5 7m5 4v6m4-6v6m1-10V4a1 1 0 00-1-1h-4a1 1 0 00-1 1v3M4 7h16" /></svg></button>
      </div>

      <div className="mt-12 space-y-3 border-t border-[#1a1a1c] pt-10 max-w-3xl mx-auto">
         <div className="space-y-1">
           <label className="text-[10px] text-zinc-600 font-bold uppercase tracking-widest px-1">Scenario Progression</label>
           <input 
              className="pc-input text-[12px] bg-transparent border-[#222] focus:border-zinc-700" 
              placeholder="(Optional) What should happen next? (e.g. 'A sudden thunderstorm begins')" 
         ...
         <div className="relative group">
           <label className="text-[10px] text-zinc-600 font-bold uppercase tracking-widest px-1">Writer Constraints</label>
           <textarea 
              className="pc-input text-[12px] min-h-[60px] bg-transparent border-[#222] focus:border-zinc-700" 
              placeholder="(Optional) Specific writing instructions: tone, style, emoji usage, etc." 
              value={activeChat.writingInstructions || ''}
              onChange={e => onUpdateChat({...activeChat, writingInstructions: e.target.value})}
              onBlur={() => handleSync(activeChat, activeChar)}
           />
           <div className="flex items-center gap-2 absolute bottom-2 right-3 text-[10px] text-zinc-600 font-bold uppercase select-none cursor-pointer">
              <input 
                type="checkbox" 
                id="long-res"
                ...
                onBlur={() => handleSync(activeChat, activeChar)}
              />
              <label htmlFor="long-res" className="cursor-pointer">long responses</label>
           </div>
         </div>
      </div>

      <div className="flex justify-center gap-3 mt-10">
         <button className="pc-btn flex items-center gap-2 text-[11px] px-6 py-2 border-[#333]"><svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M8 7H5a2 2 0 00-2 2v9a2 2 0 002 2h14a2 2 0 002-2V9a2 2 0 00-2-2h-3m-1 4l-3 3m0 0l-3-3m3 3V4" /></svg> save</button>
         <button className="pc-btn flex items-center gap-2 text-[11px] px-6 py-2 border-[#333]"><svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M3 7v10a2 2 0 002 2h14a2 2 0 002-2V9a2 2 0 00-2-2h-6l-2-2H5a2 2 0 00-2 2z" /></svg> load</button>
      ...
      {/* SYNC OK STATUS indicator as requested */}
      <div className="flex justify-center mt-12">
        <div className={`text-[10px] font-bold uppercase tracking-[0.2em] px-5 py-1.5 rounded-full border flex items-center gap-2.5 transition-all duration-500 ${
          syncStatus === 'synced' ? 'border-green-900/40 bg-green-900/5 text-green-600' : 
          syncStatus === 'syncing' ? 'border-blue-900/40 bg-blue-900/5 text-blue-500' : 
          syncStatus === 'error' ? 'border-red-900/40 bg-red-900/5 text-red-600' : 
          'border-zinc-900 bg-zinc-900/5 text-zinc-600'
        }`}>
          <span className={`w-2 h-2 rounded-full shadow-[0_0_8px_currentColor] ${syncStatus === 'synced' ? 'bg-green-500' : syncStatus === 'syncing' ? 'bg-blue-500 animate-pulse' : 'bg-zinc-700'}`}></span>
          {syncStatus === 'synced' ? '● sync ok' : syncStatus === 'syncing' ? '● syncing' : syncStatus === 'error' ? '● sync error' : '● local mode'}
        </div>
      </div>
      
      <footer className="text-center py-20 mt-10">
        <p className="text-[10px] text-zinc-800 font-bold uppercase tracking-[0.8em] select-none">DreamSpeak RP Engine • v2.5</p>
      </footer>
    </div>
  );
};
